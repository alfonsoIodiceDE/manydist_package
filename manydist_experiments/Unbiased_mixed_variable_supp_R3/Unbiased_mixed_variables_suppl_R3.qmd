...
method_levels <- c("baseline", "naive", "hl", "hl_add", "gower","gudmm", "dkss", "mod_gower", "u_ind", "u_dep", "u_mix")
...
my_colors <- c(
  ...
  "gudmm"       = "#FF7F0E",  # orange
  "dkss"        = "#7F7F7F",  # darker grey (more visible)
...
- `dkss`: Distance using kernel product similarity as proposed by @GT:2024
...
nonadd_methods <- c("naive","hl","gudmm","dkss")

# add_methods <- c("gower", "mod_gower", "hl_add", "u_ind", "u_dep", "u_mix")

legend_breaks <- c(
  c("gower","mod_gower","hl_add","u_ind","u_dep","u_mix"),  # additive first
  c("naive","hl","gudmm","dkss")                           # non-additive after
)
...
loo_mad_rel_df <- meas_to_plot_mad_rel_mean |>
  ...
  mutate(
    method = factor(method, levels = c("naive","hl","hl_add","gower","gudmm","dkss","mod_gower","u_ind","u_dep","u_mix")),
  ...
  dplyr::left_join(
    meas_to_plot_mad_rel_sd |>
      ...
      mutate(method = factor(method, levels = c("naive","hl","hl_add","gower","gudmm","dkss","mod_gower","u_ind","u_dep","u_mix"))),
    by = c("variable","var_id","method")
  )
...
ac_df <- meas_to_plot_ac_mean |>
  ...
  mutate(
    method = factor(
      method,
      levels = c("naive","hl","hl_add","gower","gudmm","dkss","mod_gower","u_ind","u_dep","u_mix")
    ),
    additive = ifelse(as.character(method) %in% add_methods, "additive", "non-additive")
  ) |>
  dplyr::left_join(
...
method_levels <- c("naive","hl","hl_add","gower","gudmm","dkss","mod_gower","u_ind","u_dep","u_mix")
...
```{r,echo=FALSE,eval=TRUE}
ggsave(fig_4_plot, file = "figure_4_mad_importances.pdf", width = 7, height = 8)
