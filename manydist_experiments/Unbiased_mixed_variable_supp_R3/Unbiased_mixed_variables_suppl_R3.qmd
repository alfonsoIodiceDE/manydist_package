...
```{r,fig.height=8,fig.width=8}
loo_mad_rel_plot <- meas_to_plot_mad_rel_mean |> 
  rename(dkps = `dkss`) |> 
  dplyr::select(variable, var_id, any_of(method_levels)) |> 
  pivot_longer(cols = naive:u_mix, names_to="method", values_to="value") |>
  mutate(
...
ac_plot <- meas_to_plot_ac_mean |> 
  ...
  mutate(
    method = factor(method, levels = c("baseline","naive","hl","hl_add","gower","gudmm","dkps","mod_gower","u_ind","u_dep","u_mix")),
    additive = ifelse(method %in% c("gower","mod_gower","hl_add","u_ind","u_dep","u_mix"),
                      "additive distance", "non-additive distance")
  ) |>
  dplyr::left_join(
    meas_to_plot_ac_sd |>
      rename(dkps = `dkss`) |>
...
```{r}
load(file="ari_pam_experiment_numsep_01_catsep_06.RData")

ari_plot_df_01_06 <- ari_pam_results_01_06 |>
  mutate(
    scenario = str_glue("num: {numsignal} signal + {numnoise} noise | cat: {catsignal} signal + {catnoise} noise"),
    scenario = factor(scenario, levels = unique(scenario))
  )

ari_plot_01_06 = ggplot(ari_plot_df_01_06, aes(x = method, y = ari, fill = method)) +
  geom_boxplot(
    outlier.alpha = 0.25,
    linewidth = 0.35
  ) +
...
print(ari_plot_01_06, width = 8, height = 4)
ggsave(file="ari_plot_01_06.pdf",ari_plot_01_06,width=6,height=8)
```

```{r}
load(file="ari_pam_experiment_numsep_001_catsep_03.RData")
ari_plot_df_001_03 <- ari_pam_results_001_03 |>
  mutate(
    scenario = str_glue("num: {numsignal} signal + {numnoise} noise | cat: {catsignal} signal + {catnoise} noise"),
    scenario = factor(scenario, levels = unique(scenario))
  )

ari_plot_001_03 = ggplot(ari_plot_df_001_03, aes(x = method, y = ari, fill = method)) +
  geom_boxplot(
    outlier.alpha = 0.25,
    linewidth = 0.35
  ) +
...
print(ari_plot_001_03, width = 8, height = 4)
ggsave(file="ari_plot_001_03.pdf",ari_plot_001_03,width=6,height=8)
```
...
ari_plot_01_035 = ggplot(ari_plot_df_01_035, aes(x = method, y = ari, fill = method)) +
  geom_boxplot(
    outlier.alpha = 0.25,
    linewidth = 0.35
  ) +
...
print(ari_plot_01_035, width = 8, height = 4)
