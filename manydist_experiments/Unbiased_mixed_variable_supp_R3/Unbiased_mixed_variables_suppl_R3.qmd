...
```{r}
load(file="ari_pam_experiment.RData")
```

```{r}

ari_plot_df <- ari_pam_results |>
  mutate(
...
ari_plot = ggplot(ari_plot_df, aes(x = method, y = ari, fill = method)) +
  geom_boxplot(
    outlier.alpha = 0.25,
    linewidth = 0.35
  ) +
...
print(ari_plot, width = 8, height = 4)
ggsave(file="ari_plot.pdf",ari_plot,width=6,height=8)
```
