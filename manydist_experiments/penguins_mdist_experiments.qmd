...
folds <- vfold_cv(penguins_df, v = 5, strata = species)

rec_gower <- recipe(species ~ ., data = penguins_df) |>
  step_mdist(all_predictors(), preset = "gower")

rec_udep <- recipe(species ~ ., data = penguins_df) |>
...
grid_k <- tibble(neighbors = c(3, 5, 7, 9, 11))
...
collect_metrics(res_gower)
collect_metrics(res_udep)
```
