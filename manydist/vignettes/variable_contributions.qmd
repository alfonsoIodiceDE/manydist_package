...
MDS diagnostics are optional. In this first example we request a two-dimensional MDS configuration explicitly with `mds = TRUE` and `dims = 2`.
...
The output of `lovo_mdist()` always contains distance-based diagnostics. Because MDS was requested above, it also contains configuration-based diagnostics, each capturing a different aspect of the change induced by removing a variable.
