cdist_old_cran <- function(x,response=NULL,validate_x=NULL,method="tvd", commensurable = TRUE, weights=1){
  ...
  x = as_tibble(x) |> mutate(across(where(is.factor), fct_drop))
  ...
  if(length(method)==1){
    ...
    if(method %in% c("none", "st_dev","HL","cat_dis","HLeucl","mca")) {
      ...
      out_delta = cat_delta_old_cran(x=x,y=y,method_cat=method)
    ...
    delta_structure = tibble(method = method_vec) |> mutate(
      ...
      delta_tmp=map2(.x = x,.y = method,.f=~cat_delta_old_cran(x=.x,method=.y)),
