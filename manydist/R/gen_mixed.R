...
#'
#' Generates synthetic mixed datasets with controllable numerical and categorical
#' signal/noise structure and balanced cluster sizes.
#'
#' @param k_true Number of clusters
#' @param clustSizeEq Observations per cluster
#' @param numsignal Number of numerical signal variables
#' @param numnoise Number of numerical noise variables
#' @param catsignal Number of categorical signal variables
#' @param catnoise Number of categorical noise variables
#' @param q Number of categories for signal categorical variables
#' @param q_err Number of categories for categorical noise
#' @param numsep Separation for numerical signal
#' @param catsep Separation for categorical signal
#' @param seed Optional seed
#' @param error_type Error distribution ("normal" or "chisq")
#' @param error_df Degrees of freedom for chi-square noise
#' @param error_scale Scale of noise
#'
#' @return A list with elements \code{df}, \code{X_num}, \code{X_cat}, \code{y},
#' \code{num_cols}, \code{cat_cols}
#'
#' @export
gen_mixed <- function(
    k_true,
  ...
  # 1) numerical signal
  if (numsignal > 0) {
    gen_num <- clusterGeneration::genRandomClust(
      numClust     = k_true,
      sepVal       = numsep,
  ...
  if (catsignal > 0) {
    gen_cat <- clusterGeneration::genRandomClust(
      numClust     = k_true,
      sepVal       = catsep,
