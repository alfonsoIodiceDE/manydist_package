cat_delta_old_cran <- function(x, y = NULL, method = NULL, method_cat="tvd", mkw_p = 1){
  ...
  if(method %in% c("tvd", "gifi_chi2",
                   "supervised","supervised_full","matching","eskin",
                   "goodall_3","goodall_4","iof","of","lin","var_entropy","var_mutability")){

    full_delta = cat_custom_delta_old_cran(ZZod=ZZod,Z=Z,Z_y=Z_y,Z_list=Z_list,
    ...
    crs = tidyr::crossing(a=1:(nvar),b=1:nvar) %>% dplyr::filter(a!=b)
