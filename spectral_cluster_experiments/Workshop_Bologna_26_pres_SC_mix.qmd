...
starting from any distance from a cluster center $d({\bf x}_{i},{\bf c}_k)$, it is  possible to construct a probabilistic clustering model^[`r Citet(bib=wp_bologna_26_bib,'Hennig19')`] ^[`r Citet(bib=wp_bologna_26_bib,'ben2008probabilistic')`] 
...
using the previous results, the dissimilarity between two mixed-data observations 
${\bf x}_i$ and  ${\bf x}_{i'}$ can be measured as 
...
$$
X_j = \beta_{0,nm} + \sum_{k>j}^{6} \beta_{1,nm} X_k, \qquad j = 1,2,3
$$
where $\beta_{1,nm}$ take different values depending on the $m$ and $n$, the observed categories of the two signal categorical variables, respectively.
...
- [modified gower]{style="color: dodgerblue"}^[`r Citet(bib=wp_bologna_26_bib,"liu2024modified")`]: an entropy based approach that takes into account inter- and intra-variable relation
